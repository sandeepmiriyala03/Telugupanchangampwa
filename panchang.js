...
function toTeluguWeek(day) {
  return {
    ...
    "Wednesday": "బుధవారం", "Thursday": "గురువారం",
    "Friday": "శుక్రవారం", "Saturday": "శనివారం"
  }[day] || day;
}
...
function toTeluguNakshatra(nak) { return nak; }
function toTeluguYoga(yoga) { return yoga; }
function toTeluguKarana(karana) { return karana; }

function generatePanchang() {
  const selectedDate = document.getElementById("pdate").value;
  if (!selectedDate) return alert("దయచేసి తేదీ ఎంచుకోండి.");
  const date = new Date(selectedDate);
  panchang.calculate(date, function () {
    document.getElementById("day").textContent = toTeluguWeek(panchang.Day.name);
    document.getElementById("tithi").textContent = `${toTeluguTithi(panchang.Tithi.name)} (ప్రారంభం: ${panchang.Tithi.start}, ముగింపు: ${panchang.Tithi.end})`;
    document.getElementById("nakshatra").textContent = `${toTeluguNakshatra(panchang.Nakshatra.name)} (ప్రారంభం: ${panchang.Nakshatra.start}, ముగింపు: ${panchang.Nakshatra.end})`;
    document.getElementById("yoga").textContent = `${toTeluguYoga(panchang.Yoga.name)} (ప్రారంభం: ${panchang.Yoga.start}, ముగింపు: ${panchang.Yoga.end})`;
    document.getElementById("karana").textContent = `${toTeluguKarana(panchang.Karna.name)} (ప్రారంభం: ${panchang.Karna.start}, ముగింపు: ${panchang.Karna.end})`;
